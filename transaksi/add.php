...
<body>
    ...
    $sql = $db->prepare("SELECT max(id_transaksi) FROM transaksi");
    $sql->execute();
    $datakode = $sql->fetch();
    if($datakode) {
        ...
        <div class="col-lg-6 col-lg-offset-3">
            <form action="proses.php" method="post" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" name="id_transaksi" id="id_transaksi" class="form-control" value="<?=$hasilkode?>" required>
                </div>
                <div class="form-group">
                    ...
                    <select name="id_barang" id="id_barang" class="form-control" required>
                        ...
                        $id = new Barang();
                        $tampil_id = $id->tampil();
                        while($data = $tampil_id->fetch(PDO::FETCH_OBJ)) {
                            echo '<option value="'.$data->id_barang.'">'.$data->id_barang.' --- '.$data->nama_barang.' --- Harga Awal : '.$data->harga_barang.' -- Stok : '.$data->stok_barang.' </option>';
						}
                        ?>
