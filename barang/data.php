		...
		<nav class="navbar navbar-default navbar-fixed-top">
			...
			<div class="container-fluid">
				...
				<div style="margin-right: 10px;" class="navbar-btn navbar-btn-right">
					<a class="btn btn-primary update-pro" href="add.php"><i class="glyphicon glyphicon-plus"></i><span> Tambah Barang</span></a>
				</div>
				<div style="margin-right: 10px;" class="navbar-btn navbar-btn-right">
					<a class="btn btn-success update-pro" href="stok_masuk.php"><i class="glyphicon glyphicon-expand"></i><span> Stok Masuk</span></a>
				</div>
                <div style="margin-right: 10px;" class="navbar-btn navbar-btn-right">
...
<form method="post" name="proses">
        ...
        <table class="table table-striped table-bordered table-hover" id="tb_barang">
            <thead>
                <tr style="background-color:#07bffc">                    
                    ...
                    <th><i class=""></i> Harga Awal</th>
                    <th><i class=""></i> Stok</th>
                    <th><i class=""></i> Gambar</th>
                    <th><i class=""></i> Tgl. Publish</th>
            ...
            $no = 1;
            $barang = new Barang();
            $tampil_brg = $barang->tampil();
            while($data = $tampil_brg->fetch(PDO::FETCH_OBJ)) {
            ?>
                <tr style="">
                    <td><?=$no++?></td>
                    ...
                    <td>
                        ...
                        <img src="../template/assets/img/barang/<?=$data->gambar_barang?>" width="70px">				
				                 
					</td>
                    <td><?= date('d F Y', strtotime($data->tgl_publish)); ?></td>
                    <td align="center">
						<a href="add_stok.php?id=<?=$data->id_barang?>" class="btn btn-success btn-xs"><i class="glyphicon glyphicon-plus"></i> Stok</a>
                        <a href="edit.php?id=<?=$data->id_barang?>" class="btn btn-warning btn-xs"><i class="glyphicon glyphicon-edit"></i></a>
                        <a onclick="return confirm('Yakin Menghapus Data?')" href="del.php?id=<?=$data->id_barang?>" class="btn btn-danger btn-xs"><i class="glyphicon glyphicon-trash"></i></a>
                    </td>                            
                </tr> 
                <?php
            }
    ...
    <div class="well" style="background-color: #cbc9ca; margin-left:20px; margin-right:20px;">
		<center>
			Total Harga Awal : <?="Rp. ".number_format($dat[0], 2, ",",".")?>
		</center>
    </div>
</form>
