...
<body>
    ...
    if(isset($_GET['id'])) {
        $barang = new Barang();
        $edit_barang = $barang->edit(@$_GET['id']);
        $data = $edit_barang->fetch(PDO::FETCH_OBJ);
        }
    ?>
    <div class="row">
        <div class="col-lg-6 col-lg-offset-3">
            <form action="proses.php" method="post" enctype="multipart/form-data">
                <input type="hidden" name="id_barang" id="id_barang" class="form-control" value="<?=$data->id_barang?>" required>
                <div class="form-group">
                    <label for="nama_barang">Nama Barang</label>                  
                    <input type="text" name="nama_barang" id="nama_barang" class="form-control" value="<?=$data->nama_barang?>" required autofocus>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    ...
                    <input type="number" name="harga_barang" id="harga_barang" class="form-control" value="<?=$data->harga_barang?>" required>
                </div>
                <div class="form-group">
                    <label for="stok_barang"></label>
                    <input type="hidden" name="stok_barang" id="stok_barang" class="form-control" value="<?=$data->stok_barang?>">
                </div>
                <div class="form-group">
                    <label for="gambar_barang">Gambar</label>
                    <br>
                    <img src="../template/assets/img/barang/<?=$data->gambar_barang?>" width="80px" id="pict">
